MiRQuest uses third-party libraries which may be distributed under different licenses. We have attempted to list all of these third party libraries and their licenses below.

You must agree to the terms of these licenses, in addition to the MiRQuest source code license, in order to use this software.

--------------------------------------------------
Third party R libraries listed by License type
[Format: Name (Project) - URL]
--------------------------------------------------

 MIT License (http://opensource.org/licenses/MIT)
  * renv - https://cran.r-project.org/web/packages/renv/
  * vroom - https://cran.r-project.org/web/packages/vroom/
  * dplyr - https://cran.r-project.org/web/packages/dplyr/
  * tidyverse - https://cran.r-project.org/web/packages/tidyverse/
  * ggplot2 - https://cran.r-project.org/web/packages/ggplot2/
  * ggraph - https://cran.r-project.org/web/packages/ggraph/
  * bslib - https://cran.r-project.org/web/packages/bslib/
  * here - https://cran.r-project.org/web/packages/here/

GPL-2 License (http://opensource.org/licenses/GPL-2.0)
  * shiny.exe - https://cran.r-project.org/web/packages/shiny.exe/index.html
  * cowplot - https://cran.r-project.org/web/packages/cowplot/
  * miRNAtap - https://www.bioconductor.org/packages/release/bioc/html/miRNAtap.html
  * edgeR - https://bioconductor.org/packages/release/bioc/html/edgeR.html
  * pheatmap - https://cran.r-project.org/web/packages/pheatmap/
  * igraph - https://cran.r-project.org/web/packages/igraph/
  * 

GPL-3 License (http://opensource.org/licenses/GPL-3.0)
  * shiny - https://cran.r-project.org/web/packages/shiny/
  * EnhancedVolcano - https://bioconductor.org/packages/release/bioc/html/EnhancedVolcano.html
  * paletteer - https://cran.r-project.org/web/packages/paletteer/
  * ReactomePA - https://bioconductor.org/packages/release/bioc/html/ReactomePA.html

Artistic-2.0 License (http://opensource.org/licenses/Artistic-2.0)
  * org.Hs.eg.db - https://bioconductor.org/packages/release/data/annotation/html/org.Hs.eg.db.html
  * org.Mm.eg.db - https://bioconductor.org/packages/release/data/annotation/html/org.Mm.eg.db.html
  * clusterProfiler - https://bioconductor.org/packages/release/bioc/html/clusterProfiler.html

CC BY 4.0 License (https://creativecommons.org/licenses/by/4.0/)
  * reactome.db - https://bioconductor.org/packages/release/data/annotation/html/reactome.db.html

LGPL License (http://opensource.org/licenses/LGPL-2.1)
  * topGO - https://bioconductor.org/packages/release/bioc/html/topGO.html
  * DESeq2 - https://bioconductor.org/packages/release/bioc/html/DESeq2.html

--------------------------------------------------
Code from the following sources has been modified to work in miRQuest. 
[Format: Name (Project) - URL]
--------------------------------------------------
We have documented where the modified code appears in the server files.

GPL-2 License (http://opensource.org/licenses/GPL-2.0)
  * anamiR - https://bioconductor.riken.jp/packages/3.5/bioc/html/anamiR.html

MIT License (http://opensource.org/licenses/MIT)
  * nf-core/smrnaseq (https://github.com/nf-core/smrnaseq/tree/master)

--------------------------------------------------
NOTE: For the most up-to-date information on licenses, please refer to the respective package documentation on CRAN or Bioconductor.
